...
export class UnconnectedVizOverviewPage extends React.Component<
  IVizOverviewPageProps,
  IVizOverviewPageState
> {
  ...
  public componentDidUpdate(
    prevProps: IVizOverviewPageProps,
    prevState: IVizOverviewPageState
  ) {
    const { location, visualizations } = this.props;
  ...
  public render() {
    ...
    for (const vignette of vignettes) {
      for (const vizId of vignette.visualizations) {
        if (
          visualizations.find(viz => viz._id === vizId) &&
          !examples.includes(vignette)
        ) {
      ...
      <Grid centered={true} columns={2}>
        ...
        <Grid.Column textAlign={"left"}>
          ...
          <>
            ...
            <List>
              <List.Item>
                compatible with: {viz.compatibleData.join(", ")}
              </List.Item>
              <List.Item>
                citation(s):{" "}
                {viz.citations.map((citation, index) => (
                  <React.Fragment
                    key={`${viz.name.toLocaleLowerCase()}-citation-${index}`}
                  >
      ...
      <>
        ...
        <Grid.Column floated={"right"}>
          <Button basic={true} icon={true} labelPosition={"right"}>
            {
              <Link
                to={`/dataset?id=${example.dataset}${example.visualizations
                  .map(viz => `&viz=${viz}`)
                  .join("")}`}
              >
                launch example
...
export const VizOverviewPage = connect(mapStateToProps)(
