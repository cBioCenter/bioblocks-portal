...
export interface IBioblocksPortalPageProps {
  ...
  dispatchVignettesFetch(
    dataset: string,
    fetchFn: () => Promise<IVignette[]>
  ): void;
  dispatchVisualizationsFetch(
    dataset: string,
    fetchFn: () => Promise<IVisualization[]>
  ): void;
}

export class UnconnectedBioblocksPortalPage extends React.Component<
  IBioblocksPortalPageProps
> {
  ...
  public async componentDidMount() {
    ...
    dispatchVignettesFetch("vignettes", async () => {
      const vignetteFetchResult = await fetch(
        `${process.env.API_URL}/vignette`
      );
      if (!vignetteFetchResult.ok) {
        return [];
      } else {
        const response = (await vignetteFetchResult.json()) as IEveResponse<
          IVignette
        >;
    ...
    dispatchVisualizationsFetch("visualizations", async () => {
      const visualizationFetchResult = await fetch(
        `${process.env.API_URL}/visualization`
      );
      if (!visualizationFetchResult.ok) {
        return [];
      } else {
        const response = (await visualizationFetchResult.json()) as IEveResponse<
          IVisualization
        >;
        ...
        <Container id={"BioblocksPortalPage"} fluid={true}>
          ...
          <Switch>
            <Route
              ...
              strict={true}
              path={"/visualizations"}
              render={this.renderVisualizationsPage}
            />
            <Route
              exact={true}
              strict={true}
              path={"/visualizations/"}
              render={this.renderVizOverviewPage}
            />
  ...
  protected renderLandingPage = (props: RouteComponentProps) => {
    ...
    if (vignettes) {
      const tabulaVignette = vignettes.find(
        vignette => vignette._id === "bbdecade-0000-0000-0001-a1234567890b"
      );
      const springVignette = vignettes.find(
        vignette => vignette._id === "bbdecade-0000-0000-0002-a1234567890b"
      );
    ...
    return (
      <LandingPage
        {...props}
        featuredVignettes={featuredVignettes}
        featuredVisualizations={featuredVisualizations}
      />
    );
  };
